...
<script type="text/javascript">
	...
	$(function(){
		contentAddEditor = E3.createEditor("#contentAddForm [name=content]"); //初始化富文本框
		E3.initOnePicUpload();  //初始化上传图片
		$("#contentAddForm [name=categoryId]").val($("#contentCategoryTree").tree("getSelected").id);//给隐藏域的categoryId赋值
	});
	//定义函数  供提交与重置使用
	var contentAddPage  = {
			submitForm : function (){
				if(!$('#contentAddForm').form('validate')){//提交时 验证表单
					$.messager.alert('提示','表单还未填写完成!');
					return ;
				}
				contentAddEditor.sync();//富文本框 同步
